...
namespace Engine
{
    ...
    {
        // Snapshot current into previous before reading new state
        s_previousKeys = s_currentKeys;
        s_previousMouse = s_currentMouse;

        // Read keyboard state directly from SDL
        ...
        // Scroll resets every frame — it's an impulse not a held state
        s_scrollDelta = 0.0f;
    }
    ...
    void InputManager::ProcessEvent(const SDL_Event& event)
    {
        ...
        {
            int idx = event.button.button - 1; // SDL buttons are 1-indexed
