...
// Minimal flat key=value .ini reader — handles exactly the small,
// known set of keys PowerEditor writes (width/height/vsync/hz).
                               ...
                               EngineExtraConfig& extra)
{
    ...
    while (std::getline(file, line))
    {
        ...
        {
            LOG_WARN("engine_config.ini: failed to parse '{}={}'", key, value);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    // with --scene=<that path> via QProcess.
    std::string scenePathArg;
    for (int i = 1; i < argc; i++)
    {
        std::string arg = argv[i];
        const std::string prefix = "--scene=";
        if (arg.rfind(prefix, 0) == 0)
            scenePathArg = arg.substr(prefix.size());
    }
    ...
    // engine_config.ini lives next to the exe — written by PowerEditor's
    // Run Settings dialog before launching, read here to override the
    ...
    // If a scene file was passed via --scene=, try loading it.
    // Falls through to the built-in demo scene if no path was given
    // or loading fails (e.g. file missing/corrupt).
    bool loadedFromFile = false;
    if (!scenePathArg.empty())
    {
        loadedFromFile = Engine::SceneSerializer::Load(
            scene, scenePathArg, renderer.GetDevice(), renderer.GetDeviceContext());
        if (loadedFromFile)
            LOG_INFO("Loaded scene from '{}' (--scene argument).", scenePathArg);
        else
            LOG_WARN("Failed to load '{}', falling back to demo scene.", scenePathArg);
    }

    // ---- Bulbs visualizing point lights (decorative, always loaded) ----
    Engine::Mesh bulb;
    ...
    while (true)
    {
        ...
        // Queried fresh each frame — never cache entity handles across
        // a potential Load(), since Load() clears and rebuilds the registry.
        {
            auto sunView = scene.Registry().view<Engine::DirectionalLightComponent>();
            for (auto e : sunView)
            {
                sunView.get<Engine::DirectionalLightComponent>(e).Light.Intensity = lightIntensity;
